...
@section('title', 'Interest Request Log')
    ...
    <div class="container">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.5rem;flex-wrap:wrap;gap:1rem;">
            <div>
                ...
                <p style="color:#64748b;margin:0.25rem 0 0;">{{ $requests->total() }} request(s)</p>
            </div>
                    ...
                    <tbody>
                        @forelse ($requests as $r)
                            ...
                            <tr class="table-row" style="border-bottom:1px solid #e2e8f0;">
                                <td style="padding:0.75rem 1rem;color:#475569;">#{{ $r->id }}</td>
                                <td style="padding:0.75rem 1rem;color:#0f172a;font-weight:600;">
                                    {{ $r->sender->name ?? '—' }}
                                    <div style="color:#64748b;font-weight:400;font-size:0.8rem;">{{ $r->sender->email ?? '' }}</div>
                                </td>
                                <td style="padding:0.75rem 1rem;color:#0f172a;font-weight:600;">
                                    {{ $r->receiver->name ?? '—' }}
                                    <div style="color:#64748b;font-weight:400;font-size:0.8rem;">{{ $r->receiver->email ?? '' }}</div>
                                </td>
                                <td style="padding:0.75rem 1rem;color:#475569;max-width:220px;">
                                    <div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                                        {{ $r->pitch->tagline ?? $r->pitch->title ?? '—' }}
                                    </div>
                                </td>
                                <td style="padding:0.75rem 1rem;">
                                    <span style="display:inline-block;padding:0.2rem 0.6rem;border-radius:999px;background:{{ $c['bg'] }};color:{{ $c['fg'] }};font-size:0.75rem;font-weight:700;text-transform:capitalize;">{{ $st }}</span>
                                </td>
                                <td style="padding:0.75rem 1rem;color:#475569;max-width:280px;">
                                    <div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ Str::limit($r->message ?? '—', 80) }}</div>
                                </td>
                                <td style="padding:0.75rem 1rem;color:#64748b;font-size:0.85rem;white-space:nowrap;">{{ $r->created_at?->format('M d, Y') }}</td>
                            </tr>
                        @empty
