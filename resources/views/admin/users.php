    ...
    <div class="container">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.5rem;flex-wrap:wrap;gap:1rem;">
            <div>
                ...
                <p style="color:#64748b;margin:0.25rem 0 0;">Total: <?= htmlspecialchars($users->total()) ?> users</p>
            </div>
            <a href="<?= htmlspecialchars(route('admin.dashboard')) ?>" class="btn btn-outline">&larr; Dashboard</a>
        </div>

        <form method="GET" action="<?= htmlspecialchars(route('admin.users')) ?>" class="card-premium" style="padding:1.25rem;margin-bottom:1.25rem;">
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:0.75rem;align-items:end;">
                ...
                <div style="display:flex;gap:0.5rem;">
                    ...
                    <a href="<?= htmlspecialchars(route('admin.users')) ?>" class="btn btn-outline">Reset</a>
                </div>
            </div>
        </form>
                        ...
                        <?php if (count($users) > 0): ?>
                            <?php foreach ($users as $u): ?>
                                ...
                                <tr class="table-row" style="border-bottom:1px solid #e2e8f0;">
                                    <td style="padding:0.75rem 1rem;color:#475569;">#<?= htmlspecialchars($u->id) ?></td>
                                    <td style="padding:0.75rem 1rem;font-weight:600;color:#0f172a;"><?= htmlspecialchars($u->name) ?></td>
                                    <td style="padding:0.75rem 1rem;color:#475569;"><?= htmlspecialchars($u->email) ?></td>
                                    <td style="padding:0.75rem 1rem;color:#475569;text-transform:capitalize;"><?= htmlspecialchars($u->role ?? '—') ?></td>
                                    <td style="padding:0.75rem 1rem;">
                                        <span style="display:inline-block;padding:0.2rem 0.6rem;border-radius:999px;background:<?= htmlspecialchars($c['bg']) ?>;color:<?= htmlspecialchars($c['fg']) ?>;font-size:0.75rem;font-weight:700;text-transform:capitalize;"><?= htmlspecialchars($ust) ?></span>
                                    </td>
                                    <td style="padding:0.75rem 1rem;">
                                    ...
                                    <td style="padding:0.75rem 1rem;color:#64748b;font-size:0.85rem;"><?= htmlspecialchars($u->created_at?->format('M d, Y') ?? '') ?></td>
                                    <td style="padding:0.75rem 1rem;">
                                        <form method="POST" action="<?= htmlspecialchars(route('admin.users.toggle-suspend', $u)) ?>" style="margin:0;">
                                            <?= csrf_field() ?>
                                            <?php if ($u->is_suspended): ?>
