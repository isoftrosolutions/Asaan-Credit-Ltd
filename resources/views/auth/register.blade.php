...
@section('description', 'Join InvestMatch Nepal — the verified marketplace for investors and entrepreneurs.')
...
<section style="padding:3rem 0 5rem;background:#f8fafc;">
    <div class="container" style="max-width:620px;">
        ...
        <div style="background:#fff;border:1px solid #e2e8f0;border-radius:1rem;padding:2.25rem;box-shadow:0 4px 6px -1px rgba(0,0,0,0.05);">
            <form method="POST" action="{{ route('register') }}">
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.75rem;">
                        @foreach ([
                            ...
                            <label style="display:block;padding:0.95rem;border:1.5px solid #cbd5e1;border-radius:0.65rem;cursor:pointer;background:#fff;transition:all 0.15s;">
                                <input type="radio" name="role" value="{{ $r['key'] }}" {{ old('role') === $r['key'] ? 'checked' : '' }} required style="margin-right:0.5rem;accent-color:#c41e3a;">
                                <span style="font-weight:600;color:#0f172a;">{{ $r['label'] }}</span>
                                <div style="font-size:0.8rem;color:#64748b;margin-top:0.2rem;margin-left:1.5rem;">{{ $r['sub'] }}</div>
                            </label>
                        @endforeach
                    </div>
                </div>
                ...
                <div style="margin-bottom:1.5rem;">
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.75rem;">
                        @foreach ([
                            ...
                            <label style="display:block;padding:0.95rem;border:1.5px solid #cbd5e1;border-radius:0.65rem;cursor:pointer;background:#fff;transition:all 0.15s;">
                                <input type="radio" name="account_type" value="{{ $t['key'] }}" {{ old('account_type') === $t['key'] ? 'checked' : '' }} required style="margin-right:0.5rem;accent-color:#c41e3a;">
                                <span style="font-weight:600;color:#0f172a;">{{ $t['label'] }}</span>
                                <div style="font-size:0.8rem;color:#64748b;margin-top:0.2rem;margin-left:1.5rem;">{{ $t['sub'] }}</div>
                            </label>
                        @endforeach
                    </div>
                </div>
                ...
                <p style="font-size:0.8rem;color:#94a3b8;text-align:center;margin-top:1rem;line-height:1.5;">
                    ...
                    <a href="{{ route('legal') }}" style="color:#475569;text-decoration:underline;">Terms &amp; Privacy Policy</a>.
                </p>
            </form>
        </div>
        ...
        <p style="text-align:center;color:#64748b;font-size:0.95rem;margin-top:1.5rem;">
            ...
            <a href="{{ route('login') }}" style="color:#c41e3a;font-weight:600;text-decoration:none;">Log in →</a>
        </p>
    </div>
</section>
@endsection
