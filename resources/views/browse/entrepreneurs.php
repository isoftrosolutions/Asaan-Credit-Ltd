<?php $title = 'Browse Entrepreneurs' ?>
<?php $description = 'Discover verified Nepali entrepreneurs raising capital.' ?>
<?php ob_start() ?>
<section class="section-premium" style="padding-top:3rem;">
    <div class="container">
        ...
        <?php /* -- Filter bar -- */ ?>
        <form method="GET" action="<?= htmlspecialchars(route('browse.entrepreneurs') ?? '') ?>"
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:0.85rem;">
                <div>
                    ...
                    <input type="text" name="search" value="<?= htmlspecialchars(request('search') ?? '') ?>" placeholder="Tagline, company..."
                ...
                <div>
                    ...
                    <select name="sector" style="width:100%;padding:0.55rem 0.75rem;border:1px solid var(--border);border-radius:var(--radius);font-size:0.95rem;background:white;">
                        ...
                        <?php foreach ($sectors as $sector): ?>
                            <option value="<?= htmlspecialchars($sector->id ?? '') ?>" <?= request('sector') == $sector->id ? 'selected' : '' ?>><?= htmlspecialchars($sector->name ?? '') ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div>
                    ...
                    <select name="stage" style="width:100%;padding:0.55rem 0.75rem;border:1px solid var(--border);border-radius:var(--radius);font-size:0.95rem;background:white;">
                        ...
                        <?php foreach ($stages as $stage): ?>
                            <option value="<?= htmlspecialchars($stage ?? '') ?>" <?= request('stage') === $stage ? 'selected' : '' ?>>
                ...
                <div>
                    ...
                    <input type="number" name="funding_min" value="<?= htmlspecialchars(request('funding_min') ?? '') ?>" min="0"
                           style="width:100%;padding:0.55rem 0.75rem;border:1px solid var(--border);border-radius:var(--radius);font-size:0.95rem;">
                </div>
                <div>
                    ...
                    <input type="number" name="funding_max" value="<?= htmlspecialchars(request('funding_max') ?? '') ?>" min="0"
                           style="width:100%;padding:0.55rem 0.75rem;border:1px solid var(--border);border-radius:var(--radius);font-size:0.95rem;">
                </div>
                <div>
                    ...
                    <input type="text" name="product_stage" value="<?= htmlspecialchars(request('product_stage') ?? '') ?>" placeholder="e.g. live"
                           style="width:100%;padding:0.55rem 0.75rem;border:1px solid var(--border);border-radius:var(--radius);font-size:0.95rem;">
                </div>
            </div>
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:0.75rem;margin-top:1rem;">
                ...
                <div style="display:flex;gap:0.5rem;">
                    <a href="<?= htmlspecialchars(route('browse.entrepreneurs') ?? '') ?>" class="btn btn-ghost" style="font-weight:600;">Reset</a>
        ...
        <?php if ($pitches->count() === 0): ?>
            <div style="background:var(--bg-white);border:1px dashed var(--border);border-radius:var(--radius-premium-lg);padding:3rem 1.5rem;text-align:center;">
                ...
                <a href="<?= htmlspecialchars(route('browse.entrepreneurs') ?? '') ?>" class="btn btn-outline" style="font-weight:600;">Clear filters</a>
            </div>
        <?php else: ?>
            <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:1.25rem;">
                <?php foreach ($pitches as $pitch): ?>
                    ...
                    ?>
                    <a href="<?= htmlspecialchars(route('pitch.show', $pitch) ?? '') ?>" style="text-decoration:none;color:inherit;display:block;">
                        <article class="card-premium" style="height:100%;display:flex;flex-direction:column;gap:0.85rem;">
                            <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:0.5rem;">
                                <h3 style="font-size:1.05rem;font-weight:700;color:var(--dark);line-height:1.35;margin:0;">
                                    <?= htmlspecialchars($pitch->tagline ?: 'Untitled pitch') ?>
                                </h3>
                                <?php if ($isVerified): ?>
                            ...
                            <div style="font-size:0.85rem;color:var(--secondary-text);font-weight:600;"><?= htmlspecialchars($displayName ?? '') ?></div>
                            ...
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.5rem;font-size:0.85rem;color:var(--foreground);">
                                <div>
                                    ...
                                    <div style="font-weight:700;color:var(--dark);"><?= htmlspecialchars($funding ?? '') ?></div>
                                </div>
                                <div>
                                    ...
                                    <div style="font-weight:700;color:var(--dark);"><?= htmlspecialchars($equity ?? '') ?></div>
                                </div>
                            </div>

                            <div style="font-size:0.8rem;color:var(--secondary-text);margin-top:auto;">
                                <?= htmlspecialchars($owner?->province ?: 'Nepal') ?>
                            </div>
                        </article>
                    </a>
                <?php endforeach; ?>
            ...
            <div style="margin-top:2rem;">
                <?= htmlspecialchars($pitches->withQueryString()->links() ?? '') ?>
            </div>
        <?php endif; ?>
    </div>
</section>
