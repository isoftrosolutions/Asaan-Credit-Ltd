        ...
        @else
            <div style="display:flex;flex-direction:column;gap:0.65rem;">
                @foreach ($notifications as $n)
                    ...
                    @endphp
                    <article style="
                        background:{{ $isUnread ? '#FEF7F2' : 'var(--bg-white)' }};
                        border:1px solid {{ $isUnread ? '#FDBA74' : 'var(--border-light)' }};
                        border-left:4px solid {{ $isUnread ? 'var(--brand-red)' : 'transparent' }};
                        border-radius:var(--radius-premium);
                        padding:1.1rem 1.25rem;
                        display:flex;justify-content:space-between;align-items:flex-start;gap:1rem;flex-wrap:wrap;">
                        <div style="flex:1;min-width:240px;">
                            <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.35rem;flex-wrap:wrap;">
                                <span class="badge-premium" style="background:{{ $tc['bg'] }};color:{{ $tc['fg'] }};">
                                    {{ ucwords(str_replace('_', ' ', $n->type ?: 'notification')) }}
                                </span>
                                @if ($isUnread)
                            ...
                            @if ($n->body)
                                <div style="font-size:0.9rem;color:var(--secondary-text);line-height:1.5;">{{ $n->body }}</div>
                            @endif
                            @if ($n->action_url)
                                <a href="{{ $n->action_url }}" style="display:inline-block;margin-top:0.5rem;color:var(--brand-red);font-weight:600;font-size:0.88rem;text-decoration:none;">View details →</a>
                            @endif
                        </div>
