...
@section('title', 'About InvestMatch Nepal')
@section('description', 'Learn about InvestMatch Nepal — our mission, our team, and our commitment to building a trusted investment marketplace.')
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:1.5rem;">
            @foreach ([
                ...
                <div class="card-premium">
                    <h3 style="font-size:1.1rem;font-weight:700;color:#0f172a;margin-bottom:0.5rem;">{{ $v['title'] }}</h3>
                    <p style="color:#64748b;line-height:1.55;margin:0;">{{ $v['desc'] }}</p>
                </div>
            @endforeach
        </div>
    </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1.5rem;">
            @foreach ([
                ...
                <div class="card-premium" style="text-align:center;">
                    <div style="width:78px;height:78px;border-radius:50%;background:linear-gradient(135deg,#1e3a8a,#c41e3a);color:#fff;font-weight:700;font-size:1.85rem;display:flex;align-items:center;justify-content:center;margin:0 auto 1rem;">{{ $m['initial'] }}</div>
                    <div style="font-weight:700;color:#0f172a;">{{ $m['name'] }}</div>
                    <div style="font-size:0.85rem;color:#64748b;margin-top:0.2rem;">{{ $m['role'] }}</div>
                </div>
            @endforeach
        </div>
    </div>
...
<section style="background:linear-gradient(135deg,#c41e3a 0%,#7f1129 100%);color:#fff;padding:4rem 0;text-align:center;">
    <div class="container" style="max-width:680px;">
        ...
        <a href="{{ route('support') }}" class="btn btn-lg" style="background:#fff;color:#c41e3a;">Get in touch</a>
    </div>
</section>
@endsection
