<?php $title = 'How InvestMatch Works' ?>
<?php $description = "From sign-up to handshake — here's exactly how InvestMatch Nepal connects entrepreneurs with verified investors." ?>
<?php ob_start() ?>
<?php /* -- HERO -- */ ?>
        ...
        <div style="display:flex;flex-direction:column;gap:1rem;">
            <?php foreach ($steps as $i => $step): ?>
                <div style="background:#fff;border:1px solid #e2e8f0;border-radius:1rem;padding:1.75rem 2rem;display:flex;gap:1.5rem;align-items:flex-start;box-shadow:0 1px 2px rgba(0,0,0,0.04);">
                    <div style="flex-shrink:0;font-size:2.2rem;font-weight:800;color:#c41e3a;letter-spacing:-0.02em;min-width:60px;"><?= htmlspecialchars($step['num'] ?? '') ?></div>
                    <div style="flex:1;">
                        <div style="display:flex;align-items:baseline;justify-content:space-between;gap:1rem;flex-wrap:wrap;margin-bottom:0.5rem;">
                            <h3 style="font-size:1.25rem;font-weight:700;color:#0f172a;margin:0;"><?= htmlspecialchars($step['title'] ?? '') ?></h3>
                            <span style="font-size:0.78rem;color:#94a3b8;background:#f1f5f9;padding:3px 10px;border-radius:999px;font-weight:600;white-space:nowrap;"><?= htmlspecialchars($step['duration'] ?? '') ?></span>
                        </div>
                        <p style="color:#475569;line-height:1.65;margin:0;"><?= htmlspecialchars($step['desc'] ?? '') ?></p>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
            ...
            <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:1rem;">
                <?php foreach ([
                    ...
                    <li style="display:flex;gap:0.85rem;align-items:flex-start;">
                        ...
                        <span style="color:#334155;line-height:1.55;"><?= htmlspecialchars($point ?? '') ?></span>
                    </li>
                <?php endforeach; ?>
            </ul>
        </div>
...
<section style="background:linear-gradient(135deg,#c41e3a 0%,#7f1129 100%);color:#fff;padding:4rem 0;text-align:center;">
    <div class="container" style="max-width:680px;">
        ...
        <div style="display:flex;gap:0.75rem;justify-content:center;flex-wrap:wrap;">
            <a href="<?= htmlspecialchars(route('register') ?? '') ?>" class="btn btn-lg" style="background:#fff;color:#c41e3a;">Create an account</a>
            <a href="<?= htmlspecialchars(route('faq') ?? '') ?>" class="btn btn-outline btn-lg" style="border-color:rgba(255,255,255,0.4);color:#fff;">Read the FAQs</a>
        </div>
    </div>
</section>
