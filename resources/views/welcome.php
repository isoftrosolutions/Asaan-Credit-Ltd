<?php
$title = 'InvestMatch Nepal — Verified Investor & Entrepreneur Marketplace';
$description = "Nepal's premium verified marketplace connecting entrepreneurs with qualified investors. Browse pitches, send interest requests, and connect when matched.";
?>
<?php ob_start() ?>
<?php
    ...
    <div class="container" style="display:grid;grid-template-columns:1.1fr 1fr;gap:3rem;align-items:center;">
        <div class="hero-content-left">
            ...
            <div class="hero-actions">
                <a href="<?= htmlspecialchars(route('register') ?? '') ?>" class="btn btn-primary btn-lg">Get Started Free</a>
                <a href="<?= htmlspecialchars(route('how-it-works') ?? '') ?>" class="btn btn-outline btn-lg">How it Works</a>
            </div>
...
<section class="section-premium">
    <div class="container">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:1.5rem;">
            <a href="<?= htmlspecialchars(route('register') ?? '') ?>" class="feature-card-premium" style="text-decoration:none;color:inherit;">
            ...
            <a href="<?= htmlspecialchars(route('browse.entrepreneurs') ?? '') ?>" class="feature-card-premium" style="text-decoration:none;color:inherit;">
            ...
            <a href="<?= htmlspecialchars(route('browse.investors') ?? '') ?>" class="feature-card-premium" style="text-decoration:none;color:inherit;">
...
<?php /* =====================  FEATURED PITCHES  ===================== */ ?>
<?php if ($featuredPitches->isNotEmpty()): ?>
<section class="section-premium">
    <div class="container">
        <div class="section-premium-header" style="display:flex;justify-content:space-between;align-items:end;flex-wrap:wrap;gap:1rem;margin-bottom:2rem;">
            ...
            <a href="<?= htmlspecialchars(route('browse.entrepreneurs') ?? '') ?>" class="btn btn-outline">View all pitches &#8594;</a>
        </div>

        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:1.5rem;">
            <?php foreach ($featuredPitches as $p): ?>
                <a href="<?= htmlspecialchars(route('pitch.show', $p) ?? '') ?>" class="card-premium" style="text-decoration:none;color:inherit;display:block;">
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;">
                        <?php if ($p->sector): ?>
                            <span style="background:rgba(30,58,138,0.08);color:#1e3a8a;font-size:0.7rem;font-weight:700;padding:4px 10px;border-radius:999px;text-transform:uppercase;letter-spacing:0.04em;"><?= htmlspecialchars($p->sector->name ?? '') ?></span>
                        <?php endif; ?>
                    ...
                    <h3 style="font-size:1.2rem;font-weight:700;color:#0f172a;margin-bottom:0.5rem;line-height:1.3;"><?= htmlspecialchars($p->tagline ?? '') ?></h3>
                    <?php if ($p->short_summary): ?>
                        <p style="color:#64748b;font-size:0.9rem;margin-bottom:1.25rem;line-height:1.5;"><?= htmlspecialchars(\Illuminate\Support\Str::limit($p->short_summary, 110) ?? '') ?></p>
                    <?php endif; ?>
                    <div style="display:flex;gap:1rem;border-top:1px solid #e2e8f0;padding-top:1rem;">
                        <div>
                            ...
                            <div style="font-weight:700;color:#0f172a;">NPR <?= htmlspecialchars(number_format((float) $p->funding_amount) ?? '') ?></div>
                        </div>
                        <?php if ($p->equity_offered): ?>
                            <div>
                                <div style="font-size:0.7rem;color:#94a3b8;text-transform:uppercase;letter-spacing:0.05em;font-weight:600;">Equity</div>
                                <div style="font-weight:700;color:#0f172a;"><?= htmlspecialchars(rtrim(rtrim(number_format((float) $p->equity_offered, 2), '0'), '.') ?? '') ?>%</div>
                            </div>
                        <?php endif; ?>
                    </div>
                </a>
...
<?php /* =====================  FEATURED INVESTORS  ===================== */ ?>
<?php if ($featuredInvestors->isNotEmpty()): ?>
<section class="section-premium" style="background:#f8fafc;">
    <div class="container">
        <div class="section-premium-header" style="display:flex;justify-content:space-between;align-items:end;flex-wrap:wrap;gap:1rem;margin-bottom:2rem;">
            ...
            <a href="<?= htmlspecialchars(route('browse.investors') ?? '') ?>" class="btn btn-outline">See all investors &#8594;</a>
        </div>

        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:1.5rem;">
            <?php foreach ($featuredInvestors as $u): ?>
                <a href="<?= htmlspecialchars(route('investor.show', $u) ?? '') ?>" class="card-premium" style="text-decoration:none;color:inherit;display:block;">
                    <div style="display:flex;align-items:center;gap:0.85rem;margin-bottom:1rem;">
                        <div style="width:48px;height:48px;border-radius:50%;background:linear-gradient(135deg,#1e3a8a,#c41e3a);color:#fff;font-weight:700;display:flex;align-items:center;justify-content:center;font-size:1.1rem;">
                            <?= htmlspecialchars(strtoupper(substr($u->company_name ?: $u->name, 0, 1)) ?? '') ?>
                        </div>
                        <div style="flex:1;min-width:0;">
                            <div style="font-weight:700;color:#0f172a;"><?= htmlspecialchars($u->company_name ?: $u->name ?? '') ?></div>
                            <?php if ($u->company_name): ?>
                                <div style="font-size:0.8rem;color:#64748b;"><?= htmlspecialchars($u->name ?? '') ?></div>
                            <?php endif; ?>
                        </div>
                    </div>
                    <?php if ($u->bio): ?>
                        <p style="color:#64748b;font-size:0.9rem;line-height:1.5;margin-bottom:1rem;"><?= htmlspecialchars(\Illuminate\Support\Str::limit($u->bio, 140) ?? '') ?></p>
                    <?php endif; ?>
                    <?php if ($u->investorProfile && $u->investorProfile->ticket_min): ?>
                        <div style="border-top:1px solid #e2e8f0;padding-top:0.85rem;font-size:0.85rem;">
                            ...
                            <span style="color:#0f172a;font-weight:700;">NPR <?= htmlspecialchars(number_format((float) $u->investorProfile->ticket_min) ?? '') ?><?php if ($u->investorProfile->ticket_max): ?> &ndash; <?= htmlspecialchars(number_format((float) $u->investorProfile->ticket_max) ?? '') ?><?php endif; ?></span>
                        </div>
                    <?php endif; ?>
                </a>
            <?php endforeach; ?>
...
<section class="section-premium">
    <div class="container">
        ...
        <div style="max-width:780px;margin:0 auto;">
            <?php foreach ([
                ...
                <div style="display:flex;gap:1.25rem;padding:1.25rem 0;border-bottom:1px solid #e2e8f0;">
                    <div style="flex-shrink:0;width:44px;height:44px;border-radius:50%;background:linear-gradient(135deg,#c41e3a,#9a1530);color:#fff;font-weight:700;display:flex;align-items:center;justify-content:center;"><?= htmlspecialchars($i + 1 ?? '') ?></div>
                    <div>
                        <h3 style="font-size:1.1rem;font-weight:700;color:#0f172a;margin-bottom:0.35rem;"><?= htmlspecialchars($step['title'] ?? '') ?></h3>
                        <p style="color:#64748b;line-height:1.55;margin:0;"><?= htmlspecialchars($step['desc'] ?? '') ?></p>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
        ...
        <div style="text-align:center;margin-top:2.5rem;">
            <a href="<?= htmlspecialchars(route('how-it-works') ?? '') ?>" class="btn btn-outline">Read full guide &#8594;</a>
        </div>
    </div>
</section>
        ...
        <div style="display:flex;flex-direction:column;gap:0.75rem;">
            <?php foreach ($faqs as $faq): ?>
                <details style="background:#fff;border:1px solid #e2e8f0;border-radius:0.75rem;padding:1.1rem 1.35rem;transition:all 0.2s;">
                    ...
                    <p style="color:#475569;line-height:1.6;margin-top:0.85rem;padding-top:0.85rem;border-top:1px solid #f1f5f9;"><?= htmlspecialchars($faq->answer ?? '') ?></p>
                </details>
            <?php endforeach; ?>
        </div>

        <div style="text-align:center;margin-top:2rem;">
            <a href="<?= htmlspecialchars(route('faq') ?? '') ?>" class="btn btn-outline">Read all FAQs &#8594;</a>
        </div>
    </div>
</section>
<?php endif; ?>
...
<section style="background:linear-gradient(135deg,#c41e3a 0%,#7f1129 100%);color:#fff;padding:4rem 0;text-align:center;">
    <div class="container" style="max-width:720px;">
        ...
        <div style="display:flex;gap:0.75rem;justify-content:center;flex-wrap:wrap;">
            <a href="<?= htmlspecialchars(route('register') ?? '') ?>" class="btn btn-lg" style="background:#fff;color:#c41e3a;">Create your account</a>
            <a href="<?= htmlspecialchars(route('how-it-works') ?? '') ?>" class="btn btn-outline btn-lg" style="border-color:rgba(255,255,255,0.4);color:#fff;">Learn more</a>
        </div>
    </div>
</section>
